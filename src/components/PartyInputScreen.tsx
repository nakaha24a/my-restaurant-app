...
const PartyInputScreen: React.FC<PartyInputProps> = ({ onStartOrder }) => {
  // 状態を `number | string` に変更して、入力欄が空の状態も扱えるようにします。
  ...
  const handleMemberCountChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // 入力が空文字列、または数値であることを確認
    if (value === "" || /^[0-9\b]+$/.test(value)) {
      setMemberCount(value);

      // 有効な数値の場合にのみ、メンバーリストを更新します。
  ...
  const handleSubmit = () => {
    // メンバー数が0または空の場合に備え、最終的なカウントを決定します。
    const finalCount =
  ...
  return (
    <div className="screen party-input-screen">
      ...
      <div className="form-group">
        <label>人数：</label>
        <input
          type="text" // ここを'text'に変更することで、より柔軟に制御できます。
          min="1"
          value={memberCount}
